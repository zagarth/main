...
require_once 'includes/JewelryDatabase.php';
require_once 'image_loader_v2.php';
...
<title>Bands Collection [TEST] - Cadman Manufacturing</title>
...
// Load Celtic patterns mapping for proper grouping
function loadCelticPatterns() {
    $patterns = [];
    ...
    if (file_exists($xmlFile)) {
        ...
        foreach ($xml->pattern as $pattern) {
            ...
            foreach ($pattern->band as $band) {
                ...
                $patternInfo['widths'][$width] = $productId;
                
                // Create reverse lookup: product_id -> pattern info
                $patterns[$productId . 'M'] = $patternInfo;
                $patterns[$productId . 'L'] = $patternInfo;
            }
        }
    }
...
// Get Celtic pattern info for a product
function getCelticPatternInfo($itemCode) {
    static $celticPatterns = null;
    if ($celticPatterns === null) {
        $celticPatterns = loadCelticPatterns();
    }
    
    // Extract base product ID (remove _alt suffixes, keep M/L)
    $baseCode = preg_replace('/(_alt\d*|_view\d*|_art\d*).*$/', '', $itemCode);
    
    return $celticPatterns[$baseCode] ?? null;
}
...
function generateBandPrice($category, $imagePath = null) {
    $basePrices = [
        'celtic' => 950,
        'cultural' => 875,
        'fancy' => 1050,
    ...
    $basePrice = $basePrices[$category] ?? 850;
    $variation = rand(-100, 250);
    return max(500, $basePrice + $variation);
}

function getSeriesDisplayName($baseName, $category) {
    if ($category !== 'plain') return null;
    
    $seriesMap = [
        'TRADITIONAL' => 'Traditional',
        'TIFFANY' => 'Tiffany',
        'COMFORT' => 'Comfort Curve',
        'RECTANGULAR' => 'Rectangular',
        'PREMIUM' => 'Premium Series'
    ];
    
    foreach ($seriesMap as $key => $display) {
        if (stripos($baseName, $key) !== false) {
            return $display;
        }
    }
...
// Get bands data from database
try {
    $bandsItems = $jewelryDB->getItems('bands');
    error_log("Database returned " . count($bandsItems) . " bands items");
} catch (Exception $e) {
    error_log("Failed to get bands items: " . $e->getMessage());
    $bandsItems = [];
}

// Group database items by category and process for display
$categories = [
    'celtic' => [
...
// Process database items into display format
$allItems = [];
$totalItems = 0;

// Group items by base name and category (handling variants and Celtic patterns)
$groupedItems = [];
foreach ($bandsItems as $item) {
    $categoryKey = $item['category_key'];
    
    // Extract filename from file_path
    $imageFilename = basename($item['file_path']);
    
    if ($categoryKey === 'celtic') {
        // Special handling for Celtic bands - group by pattern
        $patternInfo = getCelticPatternInfo($item['item_code']);
        if ($patternInfo) {
            $groupKey = $patternInfo['name']; // Group by pattern name
            $baseName = $patternInfo['name']; // Use pattern name as display name
        } else {
            // Fallback for Celtic items without pattern mapping
            $baseName = $imageFilename;
            $baseName = preg_replace('/\.(png|jpg|jpeg)$/i', '', $baseName); // Remove extension
            $baseName = preg_replace('/_alt\d*$/', '', $baseName); // Remove _alt1, _alt2, etc.
            $baseName = preg_replace('/[ML]$/', '', $baseName); // Remove size suffix
            $groupKey = $baseName;
        }
    } else {
        // For non-Celtic bands, group by base name (remove size and variant suffixes)
        $baseName = $imageFilename;
        $baseName = preg_replace('/\.(png|jpg|jpeg)$/i', '', $baseName); // Remove extension
        $baseName = preg_replace('/_alt\d*$/', '', $baseName); // Remove _alt1, _alt2, etc.
        $baseName = preg_replace('/[ML]$/', '', $baseName); // Remove size suffix
    ...
    $baseName = preg_replace('/-alt\d*$/', '', $baseName); // Remove -alt1, -alt2, etc.
    
    if (!isset($groupedItems[$categoryKey])) {
        $groupedItems[$categoryKey] = [];
    }
    
    if (!isset($groupedItems[$categoryKey][$groupKey])) {
        $groupedItems[$categoryKey][$groupKey] = [
            'baseName' => $baseName,
            'seriesBaseName' => $baseName,
            'firstProductId' => $item['item_code'],
            'mainImage' => $imageFilename,
            'variants' => [],
            'variantCount' => 0,
            'category' => $categoryKey,
            'patternInfo' => $categoryKey === 'celtic' ? $patternInfo : null
        ];
    }
    
    // Add this item as a variant
    $groupedItems[$categoryKey][$groupKey]['variants'][] = $imageFilename;
    $groupedItems[$categoryKey][$groupKey]['variantCount']++;
    
    // Use the main image (non-alt) as the primary image, or first one if no main found
    if (strpos($imageFilename, '_alt') === false && strpos($imageFilename, '-alt') === false) {
        $groupedItems[$categoryKey][$groupKey]['mainImage'] = $imageFilename;
        $groupedItems[$categoryKey][$groupKey]['firstProductId'] = $item['item_code'];
    }
}

// Convert grouped items to display format
foreach ($categories as $categoryKey => $category) {
    if (!isset($groupedItems[$categoryKey])) continue;
    
    $categoryItems = $groupedItems[$categoryKey];
    error_log("Category: $categoryKey, Grouped items: " . count($categoryItems) . " (from " . count(array_filter($bandsItems, function($item) use ($categoryKey) { return $item['category_key'] === $categoryKey; })) . " database records)");
    ...
    foreach ($categoryItems as $item) {
        $allItems[] = [
            ...
            'seriesBaseName' => $item['seriesBaseName'],
...
echo "<!-- Database-driven Bands Test Page -->\n";
echo "<!-- Total items generated: $totalItems -->\n";
echo "<!-- Grouped items by category: -->\n";
foreach ($groupedItems as $cat => $items) {
    echo "<!-- $cat: " . count($items) . " unique items -->\n";
}
echo "<!-- Sample items: -->\n";
foreach (array_slice($allItems, 0, 3) as $i => $item) {
    $patternNote = isset($item['patternInfo']) && $item['patternInfo'] ? ' (Pattern: ' . $item['patternInfo']['name'] . ')' : '';
    echo "<!-- Item $i: baseName={$item['baseName']}, firstProductId={$item['firstProductId']}, category={$item['category']}$patternNote -->\n";
}
?>
...
<div class="bands-header">
    <div class="collection-header">
        <h1>Wedding Bands Collection <span style="color: #007bff; font-size: 0.7em;">[DATABASE TEST]</span></h1>
        ...
        <p><strong>Database Status:</strong> Loaded <?php echo $totalItems; ?> items from database</p>
    </div>
</div>
...
function openConfigurator(productId, category) {
    ...
    // For Celtic bands, keep the M/L suffix but remove variant suffixes like _alt1
    if (category === 'celtic') {
        // Remove only variant suffixes (_alt1, _alt2, etc.) but keep size suffix (M/L)
        let baseProductId = productId.replace(/(_alt\d*|_view\d*|_art\d*).*$/, '');
        
        console.log('Celtic product detected - base:', baseProductId);
        
        if (typeof ProductModal !== 'undefined') {
            // Pass the category as celtic_bands to trigger the Celtic configurator
            ProductModal.open(baseProductId, { category: 'celtic_bands', allowSizing: true });
        } else {
            console.error('ProductModal not loaded, falling back to unified_detail.php');
            const url = 'unified_detail.php?collection=bands&id=' + encodeURIComponent(baseProductId) + '&category=' + encodeURIComponent(category);
            window.location.href = url;
        }
    } else {
        // For non-Celtic bands, extract base product ID by removing variant suffixes only
        let baseProductId = productId.replace(/(_alt\d*|_view\d*|_art\d*).*$/, '');
        
        // Map our category keys to ProductModal categories
        let modalCategory;
        switch(category) {
            case 'cultural': modalCategory = 'cultural_bands'; break;
            case 'fancy': modalCategory = 'fancy_bands'; break;
            case 'plain': modalCategory = 'plain_bands'; break;
            default: modalCategory = category;
        }
        
        console.log('Non-Celtic product - base:', baseProductId, 'modalCategory:', modalCategory);
        
        if (typeof ProductModal !== 'undefined') {
            ProductModal.open(baseProductId, { category: modalCategory });
        } else {
            console.error('ProductModal not loaded, falling back to unified_detail.php');
            const url = 'unified_detail.php?collection=bands&id=' + encodeURIComponent(baseProductId) + '&category=' + encodeURIComponent(category);
            window.location.href = url;
