...
static void detect_handler(AppSpeech *self)
{
    esp_afe_sr_data_t *afe_data = self->afe_data;
...
void MIC_Speech_init() 
{
    ...
    xTaskCreatePinnedToCore((TaskFunction_t)feed_handler, "App/SR/Feed", 4 * 1024, &MIC_Speech, 5, &feed_task_handle, 0);
    xTaskCreatePinnedToCore((TaskFunction_t)detect_handler, "App/SR/Detect", 5 * 1024, &MIC_Speech, 5, &detect_task_handle, 0);
    speech_running = true;
}

// Stop speech recognition to free I2S for recording
void MIC_Speech_stop(void) {
    if (!speech_running) return;
...
// Restart speech recognition after recording
void MIC_Speech_start(void) {
    if (speech_running) return;
    
    ESP_LOGI(TAG, "Restarting speech recognition");
    ...
    speech_running = true;
    ESP_LOGI(TAG, "Speech recognition restarted");
}

// Check if speech recognition is running
bool MIC_Speech_is_running(void) {
    return speech_running;
}
