...
void LVGL_Init(void)
{
    ...
    ESP_LOGI(TAG_LVGL, "=== LVGL_Init START ===");
    lv_init();
    ESP_LOGI(TAG_LVGL, "lv_init() complete");
    ...
    // Clear buffers to prevent garbage data artifacts on first render
    memset(buf1, 0, LVGL_BUF_LEN * sizeof(lv_color_t));
    memset(buf2, 0, LVGL_BUF_LEN * sizeof(lv_color_t));
    ...
    // Set screen to black - will be drawn by main loop
    lv_obj_set_style_bg_color(lv_scr_act(), lv_color_black(), 0);
    
    int64_t end_time = esp_timer_get_time();
    ESP_LOGI(TAG_LVGL, "=== LVGL_Init COMPLETE - Took %lld ms ===",(end_time - start_time) / 1000);
}
