...
int QSPI_Init(void){
  ...
  // Fill display GRAM with black to clear boot garbage
  uint16_t *black = (uint16_t *)heap_caps_calloc(EXAMPLE_LCD_WIDTH * 5, sizeof(uint16_t), MALLOC_CAP_DMA);
  if (black) {
    for (int y = 0; y < EXAMPLE_LCD_HEIGHT; y += 5) {
      esp_lcd_panel_draw_bitmap(panel_handle, 0, y, EXAMPLE_LCD_WIDTH, y + 5, black);
  ...
  // test_draw_bitmap(panel_handle);  // DISABLED - hangs with large SPI transfer size
  return 1;
}
...
void LCD_Display_On_Off(bool on)
{
    if (panel_handle) {
