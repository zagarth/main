...
bool SPD2010_Init() {
  ...
  const esp_lcd_panel_io_spi_config_t io_config ={
    ...
    .flags = {                          
      .dc_low_on_data = 0,            
      .octal_mode = 0,                
      // .quad_mode = 1,  // Not available in this ESP-IDF version               
      // .sio_mode = 0,  // Not available
      .lsb_first = 0,                 
      // .cs_high_active = 0,  // Not available          
    },
  };
  ...
  esp_lcd_panel_dev_config_t panel_config={
    ...
    // .rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB,  // Not available              
    // .data_endian = LCD_RGB_DATA_ENDIAN_BIG,      // Not available                    
...
void Backlight_Init()
{
  ledcSetup(0, Frequency, Resolution);    // Setup channel 0
  ledcAttachPin(LCD_Backlight_PIN, 0);   // Attach pin to channel 0
  ledcWrite(0, Dutyfactor);              // Set brightness    
...
void Set_Backlight(uint8_t Light)                        //
{
  if(Light > Backlight_MAX || Light < 0)
    ...
    if(Backlight == 1000)
      Backlight = 1024;
    ledcWrite(0, Backlight);  // Write to channel 0
  }
}
