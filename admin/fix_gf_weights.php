<?php
/**
 * Fix Gold Filled (GF) Weight Correction Script
 * 
 * Problem: 338 GF items have weights copied from gold instead of density-adjusted
 * Solution: Update sterling_grams = gold_grams × 0.8954 (same density ratio as sterling)
 * 
 * Usage: 
 *   - Dry run (preview only): php fix_gf_weights.php
 *   - Execute changes: php fix_gf_weights.php --execute
 */

require_once '../includes/db_config.php';

// Density ratio: Sterling (10.36 g/cm³) / 10K Gold (11.57 g/cm³)
// GF items use same ratio since they're similar density base metal with gold layer
define('DENSITY_RATIO', 0.8954);
define('STERLING_GF', 130);
define('STERLING_PRICE_PER_GRAM', 0.03215076); // GF uses same pricing as sterling/base metal

$execute = isset($argv[1]) && $argv[1] === '--execute';

echo "\n";
echo "╔════════════════════════════════════════════════════════════════╗\n";
echo "║        GOLD FILLED (GF) WEIGHT CORRECTION SCRIPT              ║\n";
echo "║        Density Ratio: 0.8954 (Same as Sterling/10K)          ║\n";
echo "╚════════════════════════════════════════════════════════════════╝\n\n";
...
try {
    ...
    // Find GF items where weight = gold weight (WRONG - needs correction)
    ...
    $stmt = $pdo->query("
        SELECT 
            gf.variant_id,
            gf.full_item_code,
            gf.sterling_grams as current_weight,
            gold.gold_grams,
            gf.selling_price,
            gf.total_cost,
            p.base_code,
            ...
            p.markup_percent,
            p.stone_cost
        FROM product_variants gf
        JOIN products p ON gf.product_id = p.product_id
        LEFT JOIN product_variants gold ON gold.product_id = p.product_id 
            AND gold.metal_type = '10K'
        WHERE gf.metal_type = 'GF'
        AND gf.sterling_grams > 0
        AND gold.gold_grams > 0
        AND ABS(gf.sterling_grams - gold.gold_grams) < 0.01
    ...
    echo "Found $totalItems GF items with incorrect weights\n\n";
    ...
    for ($i = 0; $i < min(10, $totalItems); $i++) {
        ...
        // Calculate new cost (GF uses sterling pricing)
        $newMetalCost = $newWeight * STERLING_GF * STERLING_PRICE_PER_GRAM;
        $newTotalCost = $newMetalCost + $item['labor_cost'] + $item['stone_cost'];
    ...
    if ($execute) {
        ...
        try {
            ...
            echo "\n✅ SUCCESS! Updated $updates GF items\n\n";
    ...
    if (!$execute) {
        ...
        echo "  php fix_gf_weights.php --execute\n\n";
    }
