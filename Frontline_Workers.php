...
<meta name="keywords" content="Frontline workers, firefighter jewelry, clinical services, essential workers, Cadman Manufacturing, frontline collection" />
<link rel="icon" sizes="" href="/favicon.ico">
<title>Frontline Workers Collection - Cadman Manufacturing</title>
<script src="js/jquery-3.6.0.min.js" defer></script>
</head>
<body>
    <?php include 'navigation.php'; renderNavigation('frontline_workers'); ?>
    ...
    <?php
    // Function to get base name from filename (remove variant suffixes) - same as Bands.php
    function getFrontlineBaseName($filename) {
    ...
    // Function to group images by base name - same approach as Bands.php
    function groupFrontlineImagesByBaseName($directory) {
        $images = [];
        if (is_dir($directory)) {
            ...
            foreach ($files as $file) {
                ...
                if (in_array($extension, ['png', 'jpg', 'jpeg', 'gif', 'webp'])) {
                    $baseName = getFrontlineBaseName($file);
    ...
    // Frontline-specific price generation function
    function generateFrontlinePrice($category, $filename) {
        $basePrice = 285;
    ...
    // Frontline-specific category icon function
    function getFrontlineCategoryIcon($category) {
        switch ($category) {
            case 'firefighter': return '🚒';
            case 'clinical_services': return '⚕️';
            default: return '🙏';
        }
    }
    ...
    $categories = [
        'firefighter' => [
            'path' => 'Frontline_Workers_php/images/Firefighter',
            'display_name' => 'Firefighter Collection',
            'description' => 'Professional jewelry honoring firefighters and their brave service'
        ],
        'clinical_services' => [
            'path' => 'Frontline_Workers_php/images/clinical_services',
            'display_name' => 'Clinical Services Collection',
            'description' => 'Healthcare professional jewelry celebrating clinical workers and medical staff'
        ]
    ];
    
    // Generate all items using Bands.php approach
    $allItems = [];
    $totalItems = 0;
    
    foreach ($categories as $categoryKey => $category) {
        $categoryPath = $category['path'];
        $categoryImages = groupFrontlineImagesByBaseName($categoryPath);
        ...
        foreach ($categoryImages as $item) {
            $allItems[] = [
                ...
                'price' => generateFrontlinePrice($categoryKey, $item['mainImage'])
            ];
            $totalItems++;
        }
    }
    ...
    <div class="frontline-header">
        <div class="collection-header">
            <h1>Frontline Workers Collection</h1>
            <p>Honor the heroes who serve on the frontlines every day. Our frontline workers collection celebrates the dedication, courage, and sacrifice of firefighters, healthcare professionals, and all essential workers who keep our communities safe and healthy.</p>
        </div>
    </div>
    
    <!-- Category Filter -->
    <div class="category-filter">
        <h3 style="margin-bottom: 15px; color: #8B4513;">Filter by Service</h3>
        <button class="filter-btn active" onclick="filterItems('all')">All Heroes</button>
        <button class="filter-btn" onclick="filterItems('firefighter')">🚒 Firefighter</button>
        <button class="filter-btn" onclick="filterItems('clinical_services')">⚕️ Clinical Services</button>
    </div>
            ...
            <div class="item jewelry-item paginated-item" 
                ...
                <div class="item-info">
                    <div class="category-badge">
                        <span class="category-icon"><?php echo getFrontlineCategoryIcon($item['category']); ?></span>
                    ...
                    <div class="item-actions">
                        ...
                        <button class="add-to-cart-btn btn btn-secondary" 
                                data-collection="frontline_workers"
    ...
    <script>
    // Frontline Workers pagination - wrapped in namespace to avoid conflicts
    (function() {
        let currentPage = 1;
        let itemsPerPage = 6; // Default, will be calculated dynamically
        let totalItems = 0;
        let totalPages = 1;
        let allItems = [];
        ...
        function initializeFrontlinePagination() {
        ...
        function updatePagination() {
            ...
            itemsToShow.forEach((item, index) => {
                ...
                // Stagger the animations for a nice effect like Bands.php
                setTimeout(() => {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Frontline Workers page loading...');
            
            // Initialize pagination
            setTimeout(() => {
                initializeFrontlinePagination();
                console.log('Frontline Workers pagination initialized');
            }, 100);
            ...
            console.log('Frontline Workers page initialized');
        });
    })();
    ...
    function viewDetails(itemId) {
        if (typeof ProductModal !== 'undefined') {
            ...
            window.location.href = 'unified_detail.php?collection=frontline_workers&id=' + encodeURIComponent(itemId);
        }
    }
    </script>
    ...
    <?php 
    include 'footer.php'; 
    renderFooter('frontline_workers');
    ?>
</body>
</html>
