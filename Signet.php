...
<meta name="keywords" content="Signet rings, family crests, personalized jewelry, Cadman Manufacturing, custom rings" />
<link rel="icon" sizes="" href="/favicon.ico">
<title>Signet Collection - Cadman Manufacturing</title>
<script src="js/jquery-3.6.0.min.js" defer></script>
</head>
<body>
    <?php include 'navigation.php'; renderNavigation('signet'); ?>
    ...
    <?php
    // Function to get base name from filename (remove variant suffixes) - same as Family.php and Bands.php
    function getSignetBaseName($filename) {
    ...
    // Function to group images by base name - same approach as Family.php and Bands.php
    function groupSignetImagesByBaseName($directory) {
        $images = [];
        if (is_dir($directory)) {
            ...
            foreach ($files as $file) {
                ...
                if (in_array($extension, ['png', 'jpg', 'jpeg', 'gif', 'webp'])) {
                    $baseName = getSignetBaseName($file);
    ...
    // Signet-specific price generation function
    function generateSignetPrice($category, $filename) {
        $basePrice = 450;
        ...
        if ($category === 'jewel') {
            $basePrice += 200;
        } elseif ($category === 'crest') {
            $basePrice += 150;
        }
        return $basePrice;
    }
    
    // Signet-specific category icon function
    function getSignetCategoryIcon($category) {
        switch ($category) {
            case 'crest': return '🛡';
            case 'jewel': return '💎';
            default: return '💍';
    ...
    $categories = [
        'crest' => [
            'path' => 'signet_php/images/crest_top',
            'display_name' => 'Crest Top',
            'description' => 'Traditional signet rings with custom family crest engraving and heraldic designs'
        ],
        'jewel' => [
            'path' => 'signet_php/images/jewel top',
            'display_name' => 'Jewel Top',
            'description' => 'Elegant signet rings featuring precious gemstones and sophisticated styling'
        ]
    ];
    ...
    foreach ($categories as $categoryKey => $category) {
        ...
        $categoryImages = groupSignetImagesByBaseName($categoryPath);
        ...
        foreach ($categoryImages as $item) {
            $allItems[] = [
                ...
                'price' => generateSignetPrice($categoryKey, $item['mainImage'])
            ];
            $totalItems++;
        }
    }
    ...
    foreach ($categories as $key => $category) {
        $categoryImages = groupSignetImagesByBaseName($category['path']);
    ...
    <!-- Collection Header -->
    <div class="signet-header">
        <div class="collection-header">
            <h1>Signet Collection</h1>
            <p>Discover our distinguished collection of signet rings, featuring family crests and personalized designs. Each piece represents heritage, tradition, and personal significance, crafted with meticulous attention to detail.</p>
        </div>
    </div>
    ...
    <div class="category-filter">
        ...
        <button class="filter-btn active" onclick="filterItems('all')">All Signets</button>
        <button class="filter-btn" onclick="filterItems('crest')">🛡 Crest Top</button>
        <button class="filter-btn" onclick="filterItems('jewel')">💎 Jewel Top</button>
    </div>
            ...
            <div class="item jewelry-item paginated-item" 
                ...
                <div class="item-info">
                    <div class="category-badge">
                        <span class="category-icon"><?php echo getSignetCategoryIcon($item['category']); ?></span>
    ...
    <div style="background: rgba(255,255,255,0.95); margin: 40px 20px; padding: 30px; border-radius: 10px; text-align: center; max-width: 800px; margin-left: auto; margin-right: auto; color: #333;">
        <h2 style="color: #8B4513; margin-bottom: 15px;">Heritage & Tradition</h2>
        <p style="color: #666; margin-bottom: 20px; line-height: 1.6;">
            Our signet ring collection honors centuries of tradition while embracing modern craftsmanship. From family crests that tell your story to elegant jeweled designs that capture attention, each piece is a symbol of personal heritage and distinction.
        </p>
        <a href="#formtable" style="background: linear-gradient(145deg, #FFD700, #FFA500); color: black; text-decoration: none; padding: 12px 30px; border-radius: 6px; font-size: 16px; font-weight: bold; display: inline-block; transition: all 0.3s ease;">
            Custom Signet Ring Consultation
        </a>
    </div>

    <script>
    // Signet pagination - wrapped in namespace to avoid conflicts
    (function() {
        let currentPage = 1;
        ...
        function initializeSignetPagination() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Signet page loading...');
            
            // Initialize pagination
            setTimeout(() => {
                initializeSignetPagination();
                console.log('Signet pagination initialized');
            }, 100);
            ...
            console.log('Signet page initialized');
        });
    })();
    ...
    function viewDetails(itemId) {
        if (typeof ProductModal !== 'undefined') {
            ...
            window.location.href = 'unified_detail.php?collection=signet&id=' + encodeURIComponent(itemId);
        }
    }
    </script>
